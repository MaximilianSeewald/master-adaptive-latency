...
def df_processing(df, window_size=20):
...
inputCSV = layers.Input(shape=(20,10))
csvModel = layers.LSTM(32)(inputCSV)
...
model.summary()
filepath = "./savedLSTMModel/weights-improvement-{epoch:02d}-{mae:.2f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='mae', verbose=1, save_best_only=False, mode='min')
log_dir = "./savedLSTMModel/logs/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
